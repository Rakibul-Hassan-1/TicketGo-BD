...
export function PopularRoutes() {
  ...
  useEffect(() => {
    api.get('/trips/popular-routes').then(r => setRoutes(r.data.data.routes)).catch(() => {});
  }, []);

  const handleRoute = (from: string, to: string) => {
    const date = new Date().toISOString().split('T')[0];
    router.push(`/buses/search?from=${from}&to=${to}&date=${date}&passengers=1`);
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          {routes.slice(0, 10).map(r => (
            <button key={`${r._id.from}-${r._id.to}`}
