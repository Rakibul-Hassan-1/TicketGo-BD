...
export default function LoginPage() {
  ...
  // Improve login flow: ensure axios has the token immediately after login
  // to avoid a race where components trigger requests before localStorage
  // is read and those requests return 401 and redirect back to /auth/login.
  const wrappedOnSubmit = async (data: any) => {
    const result = await dispatch(login(data));
    if (login.fulfilled.match(result)) {
      const token = (result.payload as any)?.token;
      if (token) {
        api.defaults.headers.Authorization = `Bearer ${token}`;
        // also ensure fetchMe populates latest user state if needed
        await dispatch(fetchMe()).catch(() => {});
      }
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 text-sm mt-1">
            Sign in to your TicketGo BD account
          </p>
        </div>
